    ...
    * p([H, D, d], R) = MNIW(loc, col_precision, df, scale) # matrix normal inverse wishart
    ...
    def initialize(
        ...
        if self.time_varying_emissions:
            emissions_param_ar_dependency_cov = jnp.eye(
                self.emission_dim * self.state_dim) * self.emissions_param_ar_dependency_variance
            ...
            if self.init_emissions_with_standard_normal:
                ...
                initial_emission_weights = MVN(loc=jnp.zeros(self.emission_dim * self.state_dim, ),
                                               covariance_matrix=self.num_trials * emissions_param_ar_dependency_cov).sample(
                    seed=key1)
    ...
    def marginal_log_prob(
        ...
        if add_prior_and_posterior:
            ...
            marginal_ll += MVN(params.initial_emissions.mean, params.initial_emissions.cov).log_prob(
    ...
    def log_joint(
        ...
        if self.time_varying_dynamics:
            def _compute_dynamics_lp(prev_lp, current_t):
                ...
                current_lp = prev_lp + MVN(loc=jnp.ravel(current_param),
                                           covariance_matrix=jnp.eye(
                                               self.state_dim ** 2) * self.dynamics_param_ar_dependency_variance).log_prob(
                    jnp.ravel(next_param))
                current_lp += MVN(current_param @ states[current_t], params.dynamics.cov).log_prob(
                    states[current_t + 1])
                return current_lp, None
            ...
            lp += MVN(params.initial_dynamics.mean, params.initial_dynamics.cov).log_prob(
                jnp.ravel(params.dynamics.weights[0]))
                ...
                def _compute_emissions_lp(prev_lp, current_t):
                    ...
                    current_lp = prev_lp + MVN(loc=jnp.ravel(current_param),
                                               covariance_matrix=jnp.eye(
                                                   self.emission_dim * self.state_dim) * params.emissions.ar_dependency).log_prob(
                        jnp.ravel(next_param))
                    current_lp += masks[:, current_t] * MVN(current_param @ states[current_t],
                                                            params.emissions.cov).log_prob(
                        emissions[current_t])
                    return current_lp, None

                lp, _ = jax.lax.scan(_compute_emissions_lp, lp, jnp.arange(self.sequence_length - 1))

                lp += masks[-1] * MVN(params.emissions.weights[-1] @ states[-1], params.emissions.cov).log_prob(
                    emissions[-1])
                lp += MVN(params.initial_emissions.mean, params.initial_emissions.cov).log_prob(
                    jnp.ravel(params.emissions.weights[0]))
                # lp += self.emission_prior.log_prob((params.initial_emissions.cov, params.initial_emissions.mean))
        ...
        def lgssm_params_sample(rng, stats, states, params):
            ...
            if self.fix_emissions:
                ...
                if self.time_varying_emissions:
                    ...
                    if self.emission_per_trial:
                        ...
                        if self.scan_emissions_stats:
                            def _compute_emissions_stats(carry, trial):
                                emissions_stats_1 = jnp.einsum('t,ti,jk,tl->jikl', trial, x, Rinv, x).reshape(N * D,
                                                                                                              N * D)
                                emissions_covs = jnp.linalg.inv(emissions_stats_1)
                            ...
                            emissions_stats_1 = jnp.einsum('bt,bti,jk,btl->bjikl', masks, x, Rinv, x).reshape(
                                self.num_trials, N * D, N * D)
                            emissions_covs = jnp.linalg.inv(emissions_stats_1)
                            emissions_stats_2 = jnp.einsum('bt,bti,ik,btl->bkl', masks, y, Rinv, x).reshape(
                                self.num_trials, -1)
                            emissions_y = jnp.einsum('bij,bj->bi', emissions_covs, emissions_stats_2)
                        ...
                        # expand emission weights to the original shape
                        _emissions_weights = _emissions_weights.reshape(self.num_trials, self.emission_dim,
                                                                        self.state_dim)
                        if self.orthogonal_emissions_weights:
                            ...
                            if not self.time_varying_dynamics:
                                ...
                                dynamics_stats_1 = jnp.einsum('bti,jk,btl,bt->jikl', xp, Qinv, xp,
                                                              masks[:, :-1]).reshape(D * D, D * D)
                                dynamics_stats_2 = jnp.einsum('bti,ik,btl,bt->kl', xn, Qinv, xp, masks[:, 1:]).reshape(
                                    -1)
                                dynamics_stats = (dynamics_stats_1, dynamics_stats_2)
                            else:
                                dynamics_stats = None
                        ...
                        if self.batch_update:
                            ...
                            def _update(carry, n):
                                _emissions_params = ParamsLGSSM(
                                    initial=ParamsLGSSMInitial(
                                        mean=lax.dynamic_slice(params.initial_emissions.mean, (n * D * batchN,),
                                                               (D * batchN,)),
                                        cov=lax.dynamic_slice(params.initial_emissions.cov,
                                                              (n * D * batchN, n * D * batchN),
                                                              (D * batchN, D * batchN))),
                                    ...
                                    emissions=ParamsLGSSMEmissions(
                                        ...
                                        cov=lax.dynamic_slice(params.emissions.cov, (n * batchN, n * batchN),
                                                              (batchN, batchN)),
                                        ar_dependency=None)
                                )
                                ...
                                _emissions_weights = lgssm_posterior_sample(next(rngs),
                                                                            _emissions_params,
                                                                            lax.dynamic_slice(emissions,
                                                                                              (0, n * batchN),
                                                                                              (num_timesteps, batchN)),
                            ...
                            _emissions_params = ParamsLGSSM(
                                ...
                                dynamics=ParamsLGSSMDynamics(weights=jnp.eye(self.emission_dim * self.state_dim),
                                                             bias=None,
                                                             input_weights=jnp.zeros(
                                                                 (self.emission_dim * self.state_dim, 0)),
                                                             cov=params.emissions.ar_dependency * jnp.eye(
                                                                 self.emission_dim * self.state_dim),
                                                             ar_dependency=None),
                                emissions=ParamsLGSSMEmissions(
                                    weights=jnp.kron(jnp.eye(self.emission_dim), jnp.expand_dims(x, 1)),
                                    bias=None,
                    ...
                    if self.update_emissions_param_ar_dependency_variance:
                        emissions_ar_dependency_stats_1 = (self.emission_dim * self.state_dim * (
                                    self.num_trials - 1)) / 2
                    ...
                    if self.orthogonal_emissions_weights:
                        ...
                        if not self.time_varying_dynamics:
                            ...
                            dynamics_stats_1 = jnp.einsum('bti,jk,btl,bt->jikl', xp, Qinv, xp, masks[:, :-1]).reshape(
                                D * D, D * D)
                            dynamics_stats_2 = jnp.einsum('bti,ik,btl,bt->kl', xn, Qinv, xp, masks[:, 1:]).reshape(-1)
                            dynamics_stats = (dynamics_stats_1, dynamics_stats_2)
                        else:
                ...
                if self.time_varying_dynamics:
                    ...
                    _dynamics_params = ParamsLGSSM(
                        ...
                        dynamics=ParamsLGSSMDynamics(weights=jnp.eye(self.state_dim ** 2),
                                                     ...
                                                     ar_dependency=None),
                        emissions=ParamsLGSSMEmissions(
                            weights=jnp.kron(jnp.eye(self.state_dim), jnp.expand_dims(xp, 1)),  ### kron xp
                            bias=None,
